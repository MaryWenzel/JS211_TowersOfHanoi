...
1. set up an object with 3 arrays to hold pieces
2. assign a value to each piece
3. organize pieces in descending order
   \*make a way to move pieces
4. use pop() and push() to move pieces to other arrays
5. move pieces,
   -where from?
   -where to?
   -is the move legal?
   3)check for win, if last stack equals the start length of first stack.
